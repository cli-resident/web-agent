...
| Agent | ⚠️ Частично | Инициализация и цикл опроса готовы; есть обработка `TIMEOUT`, `FILE`, `TASK/EXEC/CMD`; пул задач и graceful shutdown пока не реализованы |
...
| ЛР №3 | Сеть: регистрация, опрос задач, отправка результатов | 🚧 В процессе: базовые обработчики `FILE/TIMEOUT/TASK` реализованы, остаётся production-доработка |
