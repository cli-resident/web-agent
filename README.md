...
## Статус (обновлено 2026-03-26)

### Состояние компонентов

| Компонент | Статус | Комментарий |
|---|---|---|
| Config | ✅ Готов | Загружает и валидирует `config.json`, сохраняет `access_code`, покрыт тестами |
| Logger | ✅ Готов | Потокобезопасное логирование через spdlog + stdout и вращающийся файл |
| HttpClient | ⚠️ Частично | Работают запросы `/wa_reg`, `/wa_task`, `/wa_result`; retry/backoff и расширенные таймауты в разработке |
| Agent | ⚠️ Частично | Инициализация и цикл опроса готовы; очередь задач/`handleTask`/отправка результатов в работе |
| Тесты | ✅ Готовы для ЛР №1 | Покрывают конфигурацию, окружение и структуру репозитория |

### Этапы лабораторных работ

| ЛР | Содержание | Статус |
|---|---|---|
| ЛР №1 | Инициализация: структура, ТЗ, базовые тесты | ✅ Завершена |
| ЛР №2 | Config/Logger/HttpClient (без сети) | ⚠️ Почти завершена: остались retry и дополнительные тесты |
| ЛР №3 | Сеть: регистрация, опрос задач, отправка результатов | 🚧 В процессе: дорабатываются `handleTask` и параллельное выполнение |
| ЛР №4 | Выполнение заданий, многопоточность, итоговые интеграции | 📅 Не начата |
...
2. **Agent** — предстоящая работа включает полноценный `pollLoop`, обработку сигналов SIGINT/SIGTERM, очередь задач и `handleTask` для EXEC/CMD.
3. **Результаты задач** — предстоит формализовать JSON-ответ, автоматизировать сбор файлов из `result_directory` и добавить повторную отправку при сетевых сбоях.
